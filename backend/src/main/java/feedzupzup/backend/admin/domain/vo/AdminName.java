...
public class AdminName {
    ...
    private void validateLength(final String value) {
        if(value.isEmpty() || value.length() > MAX_LENGTH)
            throw new AdminException(ErrorCode.INVALID_ADMIN_NAME_FORMAT, "value = " + value + " length = " + value.length());
    }

    private void validateFormat(final String value) {
        if (value.contains(BLANK_SPACE)) {
            throw new AdminException(ErrorCode.INVALID_ADMIN_NAME_FORMAT, "value = " + value + " contains whitespace");
        }
    }
}
